...
class DungeonImpl implements Dungeon {
  ...
  public void move(Direction direction) {
    ...
    if (calcNextLocation(direction)) {
      if (Arrays.equals(player.getPosition().getCoordinates(), end.getCoordinates())) {
        this.gameOver = true;
      }
    }
  }
  ...
  public void shoot(Direction direction, int distance) {
    throw new IllegalStateException("Normal Dungeon will not have any arrows to shoot.");
  }
  ...
  public StringBuilder getGameDetails() {
    String temp = "";
    if (dungeon[player.getPosition().getCoordinates()[0]]
            [player.getPosition().getCoordinates()[1]].getIsTunnel()) {
      temp += " || In a Tunnel";
    } else {
      temp += " || In a Cave";
    }
    StringBuilder sb = new StringBuilder();
    sb.append("Position: ").append(Arrays.toString(player.getPosition().getCoordinates()))
            .append(temp).append(" || ").append("Bag: ").append(player.getBag())
            .append("\nPossible moves: ").append(dungeon[player.getPosition().getCoordinates()[0]]
                    [player.getPosition().getCoordinates()[1]].getNeighborList().keySet());
    return sb;
  }
  ...
  public int[] getCurrPlayerPosition() {
    return player.getPosition().getCoordinates();
  }
